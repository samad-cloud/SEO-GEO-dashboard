...
import { ForumThreadsSection } from '@/components/geo/sections/ForumThreadsSection';
...
export function ForumsTab({ crewData, region, onRefresh }: ForumsTabProps) {
  const { isRunning, lastStatus, error, progress, currentStep, start } = useCrewRun('forums', region);
  useEffect(() => { if (lastStatus === 'complete') onRefresh(); }, [lastStatus]); // eslint-disable-line react-hooks/exhaustive-deps

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium text-zinc-400 uppercase tracking-wider">Forum Threads</h2>
        <button
          onClick={start}
          disabled={isRunning}
        ...
        >
          {isRunning ? (
            ...
            <><Play className="w-3.5 h-3.5" />Run Forums Crew</>
          )}
        </button>
      </div>
      {isRunning && (
      ...
      {lastStatus === 'complete' && (
        <p className="text-xs text-green-400">Forums crew complete. Data refreshed.</p>
      )}
