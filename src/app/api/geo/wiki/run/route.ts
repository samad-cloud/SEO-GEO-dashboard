/**
 * Wiki Run API Route
 * Proxies to GEO backend: POST /api/geo/wiki/run
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${GEO_API_BASE_URL}/api/geo/wiki/run`, {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to start wiki run', details: errorText },
